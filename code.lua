...
if enabled then
    ...
    BV.MaxForce = Vector3.new(9e9, 9  HumanoidRootPart)
    BV.Velocity = Vector3.new(0, 0.1, 0)
...
-- Función mejorada para AutoAim con círculo de asistencia
local function AutoAim(enabled)
    ...
    if _G.CrosshairH then
        _G.CrosshairH:Remove()
        _G.CrosshairH = nil
    end
    if _G.CrosshairV then
        _G.CrosshairV:Remove()
        _G.CrosshairV = nil
    end
    ...
    _G.AimCircle = Drawing.new("Circle")
    _G.AimCircle.Thickness = 2
    _G.AimCircle.Color = Color3.new(1, 0, 0)
    _G.AimCircle.Transparency = 1
    _G.AimCircle.Visible = true
    _G.AimCircle.Radius = 100 -- Radio del círculo de asistencia
    ...
    _G.CrosshairH = Drawing.new("Line")
    _G.CrosshairH.Thickness = crosshairThickness
    _G.CrosshairH.Color = crosshairColor
    _G.CrosshairH.Transparency = 1
    _G.CrosshairH.Visible = true
    
    _G.CrosshairV = Drawing.new("Line")
    _G.CrosshairV.Thickness = crosshairThickness
    _G.CrosshairV.Color = crosshairColor
    _G.CrosshairV.Transparency = 1
    _G.CrosshairV.Visible = true
    ...
    _G.AimConnection = RunService.RenderStepped:Connect(function()
        -- Actualizar posición del círculo y la retícula
        local center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
        _G.AimCircle.Position = center
        
        _G.CrosshairH.From = Vector2.new(center.X - crosshairSize, center.Y)
        _G.CrosshairH.To = Vector2.new(center.X + crosshairSize, center.Y)
        
        _G.CrosshairV.From = Vector2.new(center.X, center.Y - crosshairSize)
        _G.CrosshairV.To = Vector2.new(center.X, center.Y + crosshairSize)
            ...
            if player ~= LocalPlayer and player.Character and 
               ...
               player.Character.Humanoid.Health > 0 then
                ...
                if onScreen then
                    ...
                    if isInCircle(screenPos, center, _G.AimCircle.Radius) then
...
local function Tracers(enabled)
    ...
    -- Crear tracer para cada jugador
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            tracersData[player.Name] = createTracer(player)
        end
    end
    ...
    -- Limpiar cuando los jugadores salen
    Players.PlayerRemoving:Connect(function(player)
        if tracersData[player.Name] then
            if tracersData[player.Name].tracer then
                tracersData[player.Name].tracer:Remove()
            end
            if tracersData[player.Name].connection then
                tracersData[player.Name].connection:Disconnect()
            end
            tracersData[player.Name] = nil
        end
    end)
end
